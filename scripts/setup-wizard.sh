...
# Existing dotenv state is treated as data, never sourced as shell code. This
# makes the wizard idempotent: re-running it preserves credentials unless an
# explicit argument/input variable replaces them.
existing_env() {
  [[ -f "$ENV_FILE" ]] || return 0
  node "$ROOT/scripts/env-read.mjs" "$ENV_FILE" "$1" 2>/dev/null || true
}

DOMAIN_VALUE="${DOMAIN:-$(existing_env DOMAIN)}"
ADMIN_EMAIL_VALUE="${ADMIN_EMAIL:-$(existing_env ADMIN_EMAIL)}"
ADMIN_EMAIL_VALUE="${ADMIN_EMAIL_VALUE:-admin@opensignage.local}"
XIBO_CLIENT_ID_VALUE="${XIBO_CLIENT_ID_INPUT:-$(existing_env XIBO_CLIENT_ID)}"
XIBO_CLIENT_SECRET_VALUE="${XIBO_CLIENT_SECRET_INPUT:-$(existing_env XIBO_CLIENT_SECRET)}"
AI_PROVIDER_VALUE="${AI_PROVIDER_INPUT:-$(existing_env AI_PROVIDER)}"
AI_BASE_URL_VALUE="${AI_BASE_URL_INPUT:-$(existing_env AI_BASE_URL)}"
AI_MODEL_VALUE="${AI_MODEL_INPUT:-$(existing_env AI_MODEL)}"
AI_MODEL_VALUE="${AI_MODEL_VALUE:-qwen2.5:1.5b}"
AI_API_KEY_VALUE="${AI_API_KEY_INPUT:-$(existing_env AI_API_KEY)}"
SMTP_HOST_VALUE="${SMTP_HOST_INPUT:-$(existing_env SMTP_HOST)}"
SMTP_PORT_VALUE="${SMTP_PORT_INPUT:-$(existing_env SMTP_PORT)}"
SMTP_PORT_VALUE="${SMTP_PORT_VALUE:-465}"
SMTP_USER_VALUE="${SMTP_USER_INPUT:-$(existing_env SMTP_USER)}"
SMTP_PASSWORD_VALUE="${SMTP_PASSWORD_INPUT:-$(existing_env SMTP_PASSWORD)}"
SMTP_FROM_VALUE="${SMTP_FROM_INPUT:-$(existing_env SMTP_FROM)}"
ALERT_EMAIL_VALUE="${ALERT_EMAIL_INPUT:-$(existing_env ALERT_EMAIL_TO)}"
...
Reconfiguración:
  Los valores existentes de .env se preservan por defecto. Para reemplazarlos,
  pase un argumento/variable explícito. `--ai-provider=none` deshabilita y limpia
...
if [[ "$CONFIGURE_ONLY" -eq 0 && ! -f "$ENV_FILE" ]]; then
  ...
  # Reload values generated by install.sh if they were not already supplied.
  ADMIN_EMAIL_VALUE="${ADMIN_EMAIL_VALUE:-$(existing_env ADMIN_EMAIL)}"
fi
