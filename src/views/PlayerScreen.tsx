...
export default function PlayerScreen({ token, deviceToken = '' }: { token: string; deviceToken?: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (disposed) return;
        setScene(next); offlineRef.current = false; setOffline(false); setLastSyncAt(Date.now()); retryMs.current = 5000; cacheScene(next);
      } catch {
        if (disposed) return;
        const cached = loadCached(); if (cached) setScene(cached);
    ...
    const cached = loadCached(); if (cached) setScene(cached); void load();
  ...
  return <main className="player-stage" role="main" style={{ background: scene.background || '#050b18' }} aria-label={scene.name}>
    {scene.items.map((item, index) => <PlayerItemView item={item} key={item.id || `${item.type}-${index}`} onAction={action => runAction(action, item.id || `${item.type}-${index}`)} />)}
