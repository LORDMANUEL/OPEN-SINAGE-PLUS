...
async function mockAi(page: Page) {
  ...
  await page.route('**/api/ai/generate-scene', route => route.fulfill({ json: { scene: {
    name: 'Promo IA', duration: 15, background: '#071426', items: [
      { type: 'text', text: 'Oferta IA', x: 8, y: 10, width: 84, height: 25, color: '#fff', fontSize: 64 },
      { type: 'button', text: 'Tomar turno', x: 35, y: 70, width: 30, height: 12, action: { type: 'ticket', queue: 'recepcion', prefix: 'R' } },
      { type: 'qr', value: 'https://example.com/promo', x: 75, y: 65, width: 16, height: 22 },
    ]
  } } }));
...
test.describe('Open Signage Plus V2', () => {
  ...
  test('AI Studio generates preview and only publishes after explicit approval', async ({ page }) => {
    await mockAi(page);
    await page.route('**/api/player/scenes', route => route.fulfill({ status: 201, json: { token: 'ai-player-token', scene: { name: 'Promo IA', items: [] } } }));
    await login(page);
    await page.getByRole('button', { name: 'AI Studio' }).click();
    const main = page.getByRole('main');
    await expect(main.getByText(/IA conectada/)).toBeVisible();
    await main.getByLabel('Prompt').fill('Crea una promo táctil');
    await main.getByRole('button', { name: 'Generar preview' }).click();
    await expect(main.getByText('Oferta IA')).toBeVisible();
    await expect(main.getByText(/Borrador generado/)).toBeVisible();
    await main.getByRole('button', { name: 'Aprobar y publicar' }).click();
    await expect(main.getByRole('link', { name: 'Abrir player publicado' })).toHaveAttribute('href', /ai-player-token/);
  });

  test('Queue Center issues and calls a ticket', async ({ page }) => {
    let tickets: Array<{ id: string; queue: string; prefix: string; number: string; status: string; desk?: string }> = [];
    await page.route('**/api/queues/recepcion', route => route.fulfill({ json: { tickets } }));
    await page.route('**/api/queues/recepcion/tickets', route => {
      const ticket = { id: 't1', queue: 'recepcion', prefix: 'R', number: 'R001', status: 'waiting' };
      tickets = [ticket]; return route.fulfill({ status: 201, json: { ticket } });
    });
    await page.route('**/api/queues/recepcion/call-next', route => {
      tickets = tickets.map(ticket => ({ ...ticket, status: 'called', desk: 'Módulo 1' }));
      return route.fulfill({ json: { ticket: tickets[0] } });
    });
    await login(page);
    await page.getByRole('button', { name: 'Turnos' }).click();
    const main = page.getByRole('main');
    await main.getByRole('button', { name: 'Emitir turno' }).click();
    await expect(main.getByText(/R001/)).toBeVisible();
    await main.getByRole('button', { name: 'Llamar siguiente' }).click();
    await expect(main.getByText(/Llamando R001/)).toBeVisible();
  });

  test('browser player renders QR and emits a ticket from a touch button without login', async ({ page }) => {
    await mockAi(page);
    await page.route('**/api/player/scenes/demo-player-token', route => route.fulfill({ json: { scene: {
      token: 'demo-player-token', name: 'Lobby Web', background: '#050b18', duration: 15,
      items: [
        { type: 'text', text: 'Bienvenido a Open Signage', x: 0, y: 0, width: 100, height: 40, color: '#ffffff', fontSize: 48, align: 'center' },
        { type: 'button', text: 'Tomar turno', x: 30, y: 60, width: 40, height: 15, action: { type: 'ticket', queue: 'recepcion', prefix: 'R' } },
        { type: 'qr', value: 'https://example.com', x: 75, y: 60, width: 20, height: 25, label: 'Escanea' }
      ]
    } } }));
    await page.route('**/api/queues/recepcion/tickets', route => route.fulfill({ status: 201, json: { ticket: { id: 't1', queue: 'recepcion', prefix: 'R', number: 'R001', status: 'waiting' } } }));
    await page.goto('/player/demo-player-token');
    await expect(page.getByText('Bienvenido a Open Signage')).toBeVisible();
    await expect(page.getByText('Escanea')).toBeVisible();
    await page.getByRole('button', { name: 'Tomar turno' }).click();
