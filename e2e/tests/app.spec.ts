...
async function mockAi(page: Page) {
  ...
  await page.route('**/api/ai/health', route => route.fulfill({ json: { health: { ok: true, configured: true, provider: 'ollama', model: 'tiny', latencyMs: 8, status: 200 }, diagnostics: { configured: true, provider: 'ollama', model: 'tiny', fallbackConfigured: false, metrics: { requests: 0, failures: 0, lastLatencyMs: null } } } }));
...
test.describe('Open Signage Plus V2', () => {
  ...
  test('user can log out', async ({ page }) => { await login(page); const logout = page.getByRole('button', { name: 'Cerrar sesión' }); await logout.scrollIntoViewIfNeeded(); await logout.click(); await expect(page.getByRole('heading', { name: 'Bienvenido' })).toBeVisible(); });
});
